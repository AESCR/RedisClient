...
namespace XRedis
{
    ...
    public class KetamaNodeLocator
    {
        ...
        private SortedList<long, string> oldKetamaNodes = new SortedList<long, string>();
        /// <summary>
        /// 一致性哈希的实现
        /// </summary>
        ...
        /// <param name="nodes">实际节点</param>
        /// <param name="nodeCopies">虚拟节点的个数</param>
        public void RefreshNodes(List<string> nodes, int nodeCopies)
        {
            oldKetamaNodes = new SortedList<long, string>(ketamaNodes.ToDictionary(x=>x.Key,x=>x.Value));
            ketamaNodes = null;
        ...
        public string GetOldNodeForKey(long hash)
        {
            string rv;
            long key = hash;
            //如果找到这个节点，直接取节点，返回
            if (!oldKetamaNodes.ContainsKey(key))
            {
                //得到大于当前key的那个子Map，然后从中取出第一个key，就是大于且离它最近的那个key 说明详见: http://www.javaeye.com/topic/684087
                var tailMap = from coll in oldKetamaNodes
                              where coll.Key > hash
                    select new { coll.Key };
                if (tailMap == null || tailMap.Count() == 0)
                    key = oldKetamaNodes.FirstOrDefault().Key;
                else
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public string GetOldNodes(string key)
        {
            byte[] digest = HashAlgorithm.ComputeMd5(key);
            string rv = GetOldNodeForKey(HashAlgorithm.Hash(digest, 0));
            return rv;
        }
    }
}